...
CHECKPOINT_SAVE_DIR=/bb/llm/gaf51275/llama/checkpoints/parallel/8%/next-token-shuffle/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}
...
PLAIN_TEXT_W=$((3930794675 * 2))
DATA_PATH="${DATA_PATH} $PLAIN_TEXT_W ${PRALLEL_DATASET_DIR}/default_plain_text_format_text_document"
# HIGH_QUALITY_PLAIN_TEXT_W=$((1726752028 * 2))
# DATA_PATH="${DATA_PATH} $HIGH_QUALITY_PLAIN_TEXT_W ${PRALLEL_DATASET_DIR}/highquality_plain_text_format_text_document"

# job name
JOB_NAME="llama-2-7b-base-extended-okazaki-lab-cc-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}-parallel-corpus-8%-next-token-shuffle"
