...
# model config
HIDDEN_SIZE=768
FFN_HIDDEN_SIZE=3072 # intermediate size (HuggingFace)
NUM_LAYERS=12
NUM_HEADS=12
SEQ_LENGTH=4096
...
CHECKPOINT_DIR=/bb/llm/gaf51275/llama/checkpoints/MoE/debug/mixtral-350mx8/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}
CHECKPOINT_SAVE_DIR=/bb/llm/gaf51275/llama/checkpoints/MoE/debug/mixtral-350mx8/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}
...
# job name
JOB_NAME="mixtral-125mmx8-okazaki-lab-cc-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
