...
if [ "$#" -ne 9 ]; then
    echo "Usage: $0 FFN_HIDDEN_SIZE NUM_LAYERS NUM_HEADS NUM_QUERY_GROUPS SEQ_LENGTH TENSOR_PARALLEL_SIZE PIPELINE_PARALLEL_SIZE MICRO_BATCH_SIZE GLOBAL_BATCH_SIZE"
    exit 1
fi
...
# job name
JOB_NAME="llama2-175b-base-selective-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}-HS=${HIDDEN_SIZE}-FHS=${FFN_HIDDEN_SIZE}-NL=${NUM_LAYERS}-NH=${NUM_HEADS}-NQG=${NUM_QUERY_GROUPS}-MBS=${MICRO_BATCH_SIZE}"
...
HOSTFILE_NAME=/mnt/nfs/home/llmjp0/Megatron-LM/scripts/sakura/hostfile/56node
