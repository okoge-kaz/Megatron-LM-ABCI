...
# job name
JOB_NAME="gpt-13b-base-selective-transformer-engine-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
mpirun -np $NUM_GPUS \
  ...
  -x CPATH=/mnt/nfs/home/llmjp0/.pyenv/versions/3.10.13/include/python3.10:$CPATH \
  -x LD_LIBRARY_PATH \
