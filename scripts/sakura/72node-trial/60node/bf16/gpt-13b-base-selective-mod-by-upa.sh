...
CHECKPOINT_SAVE_DIR=/mnt/nfs2/data0/work/checkpoints/gpt-13b-base-megatron/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}
...
# job name
JOB_NAME="gpt-13b-base-selective-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
HOSTFILE_NAME=/mnt/nfs/home/llmjp0/Megatron-LM/scripts/sakura/hostfile/60node
...
mpirun -np $NUM_GPUS \
  ...
  --use-mpi \
  --wandb-name ${JOB_NAME} \
  --wandb-project "sakura-h100-flops-72node" \
  --wandb-entity "llm-jp"
